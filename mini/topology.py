...
def center_and_normalize(X):
    ...
    X /= (X.norm(dim=1, keepdim=True) + 1e-10)
    return X
...
def compute_cca(X, Y, k=None):
    ...
    Ux, Sx, _ = svd(Cxx)
    Uy, Sy, _ = svd(Cyy)
    Wx = Ux @ torch.diag(1.0 / torch.sqrt(torch.from_numpy(Sx)))
    Wy = Uy @ torch.diag(1.0 / torch.sqrt(torch.from_numpy(Sy)))
...
def compute_pwcca(X, Y):
    ...
    weighted_corr = sum(w * c for w, c in zip(proj_weights, C))
